...
from ForensicsAdapter.dataset.albu import IsotropicResize
...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        try:
            tmp = self.config['dataset_json_folder']
            with open(os.path.join(self.config['dataset_json_folder'], dataset_name + '.json'), 'r') as f:
                dataset_info = json.load(f)
        except Exception as e:
            print(e)
            raise ValueError(f'dataset {dataset_name} not exist!', tmp)
    ...
    def __getitem__(self, index):
        ...
        image_path = os.path.join('dataset', self.data_dict['image'][index])
