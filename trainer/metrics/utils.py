...
def get_test_metrics(y_pred, y_true, img_names):
    ...
    cm = metrics.confusion_matrix(np.clip(y_true, a_min=0, a_max=1), prediction_class)
    ...
    return {'acc': acc, 'auc': auc, 'eer': eer, 'ap': ap, 'pred': y_pred, 'video_auc': v_auc, 'label': y_true, 'confusion_matrix': cm_list}
